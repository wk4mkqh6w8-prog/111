...
DAILY_LIMIT = 5
# --- Цены ---
PRICE_USD = 5                   # цена подписки (в долларах)
...
async def on_img_btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_premium(user_id):
        await q.message.reply_text(
            ...
            f"Нажмите «Купить подписку», стоимость {PRICE_TEXT} на 30 дней.",
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Стоимость — всего <b>{PRICE_TEXT} на 30 дней</b>.\n\n"
        "🚀 Что я умею:\n"
...
def _faq_text() -> str:
    return (
        ...
        f"  {PRICE_TEXT} за 30 дней. Оплатить можно через кнопку «Купить подписку».\n\n"
...
async def on_help_how(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = (
       ...
       f"• Премиум ({PRICE_TEXT} / 30 дней) — /buy.\n\n"
...
async def _create_crypto_invoice_link(user_id: int) -> str:
    ...
    data = {
        "asset": "USDT",
        "amount": PRICE_USDT,
        "description": f"Подписка на 30 дней ({PRICE_TEXT})",
        "payload": payload,
    }
...
async def on_buy_btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
    f"Оплата подписки на 30 дней: <b>{PRICE_TEXT}</b>\n\n"
    "<b>Премиум даёт</b>:\n"
